window.addEventListener("DOMContentLoaded", function () {
    ...
    inputs.forEach((element) => {
        element.addEventListener('blur', function (evento) {
            ...
            if (element.name === 'dni') {
                isValid = validaDNI(element.value + letraDNI.value);
            } else if (element.name === 'letradni') {
                isValid = validaDNI(numeroDni.value + element.value);
            } else if (element.name === 'edad') {
                isValid = validaEdad(element.value);
            } else if (element.name === 'telefono') {
                isValid = validaTelefono(element.value);
            } else if (element.name === 'email' || element.name === 'repeatemail') {
                isValid = validaEmail(email.value) && email.value === repeatemail.value;
            } else {
                isValid = !checkVacio(element);
            }
...
function checkVacio(campo) {
    return (campo.value == '' || campo == null || campo == undefined) ? true : false;
}
...
function pintaInput(elemento, result) {
    if (result) {
        ...
        if (document.querySelector('#errormsg')) {
            elemento.parentNode.removeChild(document.querySelector('#errormsg'))
        }
    } else {
        let msg = document.createElement('p');
        msg.id = 'errormsg';
        msg.style.color = 'red';
        msg.innerText = '* Verificar los campos marcados en rojo';
