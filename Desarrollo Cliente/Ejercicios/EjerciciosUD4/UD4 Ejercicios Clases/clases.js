  ...
  function Factura(cliente, elementos, formaPago) {
    ...
    this.calculaImporte = function () {
      this.elementos.forEach(
        (elemento) =>
          (this.total += elemento.precio * elemento.cantidad * this.iva)
      );
    };
  ...
  class Vaca extends Animal {
    ...
    ordenyar(litros) {
      if (litros > this.leche) {
        console.log(
          `No se puede ordenar ${litros} litros de leche, le quedan ${this.leche} litros`
        );
        return false;
      } else {
        this.leche -= litros;
...
function ejer8() {
  ...
  console.log(objeto);
  console.log(
    objeto.implementa("saludar", function () {
